<template>
    <div>Android SDK 下载</div>
    <div class="android-nva">
        <router-link :class="activeMenu === 'home' ? 'active' : ''" :to="`/${$route.params.layout}/home`">下载说明
        </router-link>
        <router-link :to="`/${$route.params.layout}/${item[0].android_type.toLowerCase()}`"
            :class="item[0].android_type.toLowerCase() === activeMenu ? 'active' : ''" v-for="(item, key) in androidsdk"
            :key="key">{{
                    item[0].android_type
            }}
        </router-link>
    ...
    <div class="android-content">
        ...
        <div v-else class="android-sdk">
            <div v-for="(item, key) in androidsdk[activeMenu.toUpperCase()]">
...
import { androidsdkTest, androidappTest, androiddownloadmessageTest } from "@/services";
